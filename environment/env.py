...
class QuayScheduling:
    ...
    def step(self, action):
        ...
        while True:
            ...
            if self.algorithm == "RL":
                ...
                mask = self._get_mask()
            elif "GP" in self.algorithm:
                next_state, current_ops, added_info = self._get_state_for_GP()
                mask = self._get_mask()
            else:
    ...
    def reset(self):
        ...
        while True:
            ...
            if self.algorithm == "RL":
                ...
                initial_state, current_ops, added_info = self._get_state_for_GP()
                mask = self._get_mask()
            else:
        ...
        for ship in self.monitor.ships_in_queue.values():
            ...
            for quay_id, name in self.quay_ids.items():
                ...
                if self.numerical_encoding:
                    if (priority_score != 0.0) and (not quay_id in self.actions_done):
    ...
    def _get_state_for_GP(self):
        state = np.zeros((int(self.num_of_quays * self.num_of_ships), 6))
        ...
        for ship_idx, ship_name in self.ship_ids.items():
            ...
            if operation is None:
                operation = ship.operations[-1]
            offset = int(ship_idx * self.num_of_quays)

            if operation.id in self.monitor.operations_in_quay.keys():
                operation = self.monitor.operations_in_quay[operation.id]
                state[offset:offset + self.num_of_quays, 0] = 3
            elif operation.id in self.monitor.operations_interrupted.keys():
                operation = self.monitor.operations_interrupted[operation.id]
                state[offset:offset + self.num_of_quays, 0] = 3
            elif operation.id in self.monitor.operations_in_buffer.keys():
                operation = self.monitor.operations_in_buffer[operation.id]
                state[offset:offset + self.num_of_quays, 0] = 2
            elif operation.id in self.monitor.operations_done:
                operation = self.monitor.operations_done[operation.id]
                state[offset:offset + self.num_of_quays, 0] = 0
            else:
                operation = self.monitor.operations_unscheduled[operation.id]
                state[offset:offset + self.num_of_quays, 0] = 1

            state[offset:offset + self.num_of_quays, 1] = int(operation.duration) / self.df_scenario["Duration"].max()

            if operation.interruption == "S":
                state[offset:offset + self.num_of_quays, 2] = 3
                state[offset:offset + self.num_of_quays, 3] = operation.fixed_duration / operation.duration
            elif operation.interruption == "F":
                state[offset:offset + self.num_of_quays, 2] = 2
                state[offset:offset + self.num_of_quays, 3] = operation.fixed_duration / operation.duration
            else:
                state[offset:offset + self.num_of_quays, 2] = 1
                state[offset:offset + self.num_of_quays, 3] = 0
        ...
        for quay_id, name in self.quay_ids.items():
            ...
            occupied, interruption = quay.check_status(self.sim_env.now)
            if not occupied:
                state[quay_id::self.num_of_quays, 4] = 3
                state[quay_id::self.num_of_quays, 5] = 0
            else:
                if interruption:
                    state[quay_id::self.num_of_quays, 4] = 2
                    state[quay_id::self.num_of_quays, 5] = 0
                else:
                    state[quay_id::self.num_of_quays, 4] = 1
                    remaining_duration = float('inf')
                    max_duration = 0
                    for operation in quay.operations_in_working.values():
                        remaining_duration = min(remaining_duration, operation.duration -
                                                 (operation.progress + self.sim_env.now - operation.working_start))
                        max_duration = max(max_duration, operation.duration)
                    state[quay_id::self.num_of_quays, 5] = remaining_duration / max_duration
        ...
        #         if candidate_flag:
        #             if not occupied:
        #                 state[offset + quay_idx, 6] = 2
        #             else:
        #                 state[offset + quay_idx, 6] = 1
        #
        #             if self.numerical_encoding:
        #                 state[offset + quay_idx, 7] = priority_score / 1.5
        #             else:
        #                 if priority_score in ["A", "B"]:
        #                     state[offset + quay_idx, 7] = 2
        #                 else:
        #                     state[offset + quay_idx, 7] = 1
